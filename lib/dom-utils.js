 ...
 *
 * A 404 or 500 is a perfectly successful navigation as far as CDP is concerned:
 * the error page loads, has a DOM, and produces no findings. Auditing one
 * reports "0 findings" for a page that was never rendered, so the response
 * status is checked here and treated as a failure.
 *
 * @param {Object} client - CDP client
...
async function navigate(client, url, timeout = 30000) {
  ...
  // Record the status of the document response for the frame we navigate.
  const statusByFrame = new Map();
  const onResponse = (params) => {
    if (params.type === 'Document' && params.frameId) {
      statusByFrame.set(params.frameId, params.response);
    }
  };
  client.on('Network.responseReceived', onResponse);

  try {
    const { frameId, errorText } = await client.send('Page.navigate', { url });
    ...
    const response = statusByFrame.get(frameId);
    if (response && response.status >= 400) {
      throw new Error(
        `${url} returned HTTP ${response.status}${response.statusText ? ` ${response.statusText}` : ''}; ` +
        'an error page has nothing meaningful to audit'
      );
    }
  } finally {

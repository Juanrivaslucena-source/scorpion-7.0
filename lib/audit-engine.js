...
class AuditEngine {
  ...
  async run() {
    ...
    const results = {
      ...
      findingsByRule: {},
      findingsBySeverity: {},
      // Routes that could not be audited. Tracked separately from findings: a
      // route that never rendered has zero findings, and that must not read as
      // a pass.
      routeErrors: [],
    ...
    for (const viewport of this.viewports) {
      ...
      for (const route of this.routes) {
        ...
        try {
          ...
          if (!domTree) {
            const message = 'could not read the DOM tree';
            console.warn(`Failed to audit ${url}: ${message}`);
            results.routeErrors.push({ url, viewport: viewport.name, error: message });
            viewportResults.routes.push({ url, error: message, findings: [] });
            continue;
          }
          ...
          const context = {
            ...
            getClientWidth: (node) => getClientWidth(this.client, node),
          ...
          results.routeErrors.push({ url, viewport: viewport.name, error: error.message });
          viewportResults.routes.push({
            url,
            error: error.message,
            findings: []

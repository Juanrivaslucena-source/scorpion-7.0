 ...
 */
// Only routes the audit target actually serves. /dashboard, /settings and
// /profile were listed here but do not exist in demo-site, so half of every run
// audited 404 pages and counted them as clean.
...
class AuditEngine {
  ...
  /**
   * Fetch a route's HTTP status without rendering it.
   *
   * Uses Node 22's built-in global fetch, so this adds no dependency.
   * Returns null when the status cannot be determined, which is treated as
   * "unknown" rather than "fine" — the caller only fails on a definite non-2xx.
   *
   * @param {string} url
   * @returns {Promise<number|null>}
   */
  async _checkRouteStatus(url) {
    if (typeof globalThis.fetch !== 'function') return null;
    try {
      const response = await globalThis.fetch(url, {
        method: 'GET',
        redirect: 'follow',
        signal: AbortSignal.timeout(Math.min(this.timeout, 15000)),
      });
  ...
  async run() {
    ...
    const results = {
      ...
      viewports: [],
      totalFindings: 0,
      // Denominators. "0 findings" is unreadable without them: it means
      // "clean" only if pagesAudited > 0 and pagesFailed === 0.
      pagesAudited: 0,
      pagesFailed: 0,
      failedRoutes: [],
      findingsByRule: {},
    ...
    for (const viewport of this.viewports) {
      ...
      for (const route of this.routes) {
        ...
        try {
          // Chromium renders a 404 body like any other page, so navigation
          // alone cannot distinguish a real route from a missing one. Check the
          // status first; auditing an error page and calling it clean is how
          // three nonexistent routes passed silently for the life of this tool.
          const status = await this._checkRouteStatus(url);
          ...
          if (!domTree) {
            ...
            results.pagesFailed += 1;
            results.failedRoutes.push({ url, viewport: viewport.name, reason: 'no DOM tree' });
            continue;
          }

          results.pagesAudited += 1;
          ...
          results.pagesFailed += 1;
          results.failedRoutes.push({ url, viewport: viewport.name, reason: error.message });
          viewportResults.routes.push({
            url,
            error: error.message,
            findings: []

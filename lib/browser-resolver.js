...
class BrowserResolver {
  ...
  async _findSystemBrowser() {
    const paths = [
      ...(await this._findPlaywrightBrowsers()),
      ...(SYSTEM_BROWSERS[this.platform] || [])
    ];
  ...
  /**
   * Find Chromium installed by Playwright, which many CI images preinstall and
   * point at with PLAYWRIGHT_BROWSERS_PATH. Cheaper and more reliable than
   * downloading a build at audit time.
   * @returns {Promise<string[]>} Candidate executable paths
   */
  async _findPlaywrightBrowsers() {
    const root = process.env.PLAYWRIGHT_BROWSERS_PATH;
    if (!root || this.platform === 'win32') {
      return [];
    }
    ...
    const suffix = this.platform === 'darwin'
      ? 'Chromium.app/Contents/MacOS/Chromium'
      : 'chrome-linux/chrome';

    // Prefer the full browser over the headless shell; the audit takes screenshots.
    return entries
      .filter(entry => entry.startsWith('chromium'))
      .sort((a, b) => Number(a.includes('headless')) - Number(b.includes('headless')))
      .map(entry => path.join(root, entry, suffix));
  }

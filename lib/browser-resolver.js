...
// expressed as path segments relative to the user's home directory.
const PLAYWRIGHT_DEFAULT_ROOTS = {
  darwin: [['Library', 'Caches', 'ms-playwright']],
  linux: [['.cache', 'ms-playwright']],
  win32: [['AppData', 'Local', 'ms-playwright']]
};

// Executable locations inside a Playwright chromium-<revision> directory.
...
class BrowserResolver {
  ...
  async resolve() {
    ...
    // 2. Check a Playwright browser cache (common in CI images and sandboxes)
    const playwrightPath = await this._findPlaywrightBrowser();
    if (playwrightPath) {
      console.log(`Found Playwright-managed browser: ${playwrightPath}`);
      return playwrightPath;
    }

    // 3. Check system browsers
  ...
  /**
   * Find a Chromium installed by Playwright.
   *
   * CI images and sandboxes frequently pre-install Chromium this way and set
   * PLAYWRIGHT_BROWSERS_PATH. Without this step the resolver falls through to
   * a network download that such environments usually block.
   * @returns {Promise<string|null>}
   */
  async _findPlaywrightBrowser() {
    const roots = [];
    ...
    roots.push(...(PLAYWRIGHT_DEFAULT_ROOTS[this.platform] || []).map(
      (dir) => path.join(os.homedir(), ...dir)
    ));

    const relativeExecutables = PLAYWRIGHT_EXECUTABLES[this.platform] || [];
    ...
    for (const root of roots) {
      ...
      // `chromium` is often a symlink to the newest build; prefer it, then
      // fall back to the highest-numbered chromium-<revision> directory.
      ...
      for (const candidate of candidates) {
        ...
        if (await this._exists(candidatePath)) {
          return candidatePath;
        }

        for (const relative of relativeExecutables) {
          const executable = path.join(candidatePath, relative);
          if (await this._exists(executable)) {
            return executable;
          }
        }
      }
